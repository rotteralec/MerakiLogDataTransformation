...
def field_seperator(temp_arr, _data):
    ...
    for i in _data[9:]:
        ...
        if(equal_ind != -1):
            temp_cat = str(i[0:equal_ind])
            temp_val = str(i[equal_ind+1:])
...
def event_sorter( event_data):
    ...
    if(event_data[9] == "type=8021x_eap_success"):
        field_seperator(each_log, event_data)
        #del each_log[0:1]
...
get_dir()
print(xsucarr)
pd.DataFrame(xsucarr).to_csv('xsuccess_all.csv')

#event_sorter(data)
#print(eventarr)
""" print(xautharr)
print(xdeautharr)
print(xclientdeautharr)
print(xsucarr) """
""" for i in data:
    temp = []
    parts = i.split(',')
...
#print(urlarr)
#pd.DataFrame(urlarr).to_csv('urls.csv')
#print(firewallarr)
#pd.DataFrame(firewallarr).to_csv('firewall.csv')
#print(flowsarr)
...
#print(vpnfirewallarr)
#pd.DataFrame(vpnfirewallarr).to_csv('vpnfirewall.csv')
#print(eventarr)
#pd.DataFrame(eventarr).to_csv('events_test.csv')
#pd.DataFrame(xarr).to_csv('x_test.csv')
#print(ipflowarr)
#pd.DataFrame(ipflowarr).to_csv('ipflows.csv')
#print(otherarr)
#pd.DataFrame(otherarr).to_csv('OTHERLOGS.csv')
#print(otherarr)
#print(counter)
